assert_cmd "ohmyzsh is a git work tree"  git -C _vendor/ohmyzsh rev-parse --is-inside-work-tree
assert_cmd "ohmyzsh at pinned ref" \
    bash -c '[[ -n $OHMYZSH_REF && $(git -C _vendor/ohmyzsh rev-parse HEAD) == "$OHMYZSH_REF" ]]'
assert_cmd "ohmyzsh not group-writable" \
    bash -c '[[ ! -w _vendor/ohmyzsh || $(stat -c %A _vendor/ohmyzsh | cut -c6) == "-" ]]'

# R9: zsh-autosuggestions is cloned to its real Oh My Zsh custom-plugin load
# path, not _vendor/ - Oh My Zsh only resolves custom plugins there.
...
for i in "${!cmlib_dirs[@]}"; do
    ...
    ref_val="${!ref_var}"
    assert_cmd "present: $c" test -d "_vendor/$c"
    assert_cmd "$c at pinned ref" bash -c \
        "[[ -n \"$ref_val\" && \$(git -C _vendor/$c rev-parse HEAD) == \
           \$(git -C _vendor/$c rev-parse --verify \"$ref_val^{commit}\") ]]"
done

assert_cmd "no ssh remotes anywhere" bash -c \
    '! grep -rq "git@github.com" _vendor/*/.git/config zsh/custom/plugins/zsh-autosuggestions/.git/config'
